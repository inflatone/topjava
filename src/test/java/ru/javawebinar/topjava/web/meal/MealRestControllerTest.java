...
class MealRestControllerTest extends AbstractControllerTest {
    ...
    void testUpdate() throws Exception {
        ...
        mockMvc.perform(
                put(REST_URL + MEAL1_ID).contentType(MediaType.APPLICATION_JSON).content(JsonUtil.writeValue(updated))
        ).andExpect(status().isNoContent());
    ...
    void testCreated() throws Exception {
        ...
        ResultActions action = mockMvc.perform(
                post(REST_URL).contentType(MediaType.APPLICATION_JSON).content(JsonUtil.writeValue(created))
        );
